...
function testDB() {
  ...
  if (localStorage.getItem('actionDB') !== null) {
    ...
    //This finds the first element with a #accessDB tag in HTML document and adds to its inner html with the ith game title as well as a line break
    for (let i = 0; i < 5; i++) {
      document.querySelector('#accessDB').innerHTML +=
        actionGames[i].title + '<br>';
    }

    //This justs adds another line break so we can see the changes in the next loop
    document.querySelector('#accessDB').innerHTML += '<br>';
